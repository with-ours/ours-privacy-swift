...
/// Access to the OursPrivacy People API, available as an accessible variable from
/// the main OursPrivacy instance.
open class People {

    /// controls the $ignore_time property in any subsequent OursPrivacyPeople operation.
    /// If the $ignore_time property is present and true in your request,
    /// OursPrivacy will not automatically update the "Last Seen" property of the profile.
    /// Otherwise, OursPrivacy will add a "Last Seen" property associated with the
    ...
    func addPeopleRecordToQueueWithAction(_ action: String, properties: InternalProperties) {
        ...
        serialQueue.async { [weak self, action, properties] in
            ...
            if let anonymousId = self.oursprivacyInstance?.anonymousId {
               r["$device_id"] = anonymousId
            }

            if let userId = self.oursprivacyInstance?.userId {
                r["$user_id"] = userId
            }

            if let hadPersistedDistinctId = self.oursprivacyInstance?.hadPersistedDistinctId {
            ...
            if let distinctId = self.distinctId {
                ...
                // identified
                self.oursprivacyPersistence.saveEntity(r, type: .people, flag: !PersistenceConstant.unIdentifiedFlag)
            } else {
                self.oursprivacyPersistence.saveEntity(r, type: .people, flag: PersistenceConstant.unIdentifiedFlag)
            }
        }
    ...
    /**
     Set properties on the current user in OursPrivacy People.
    ...
    /**
     Convenience method for setting a single property in OursPrivacy People.
    ...
    /**
     Set properties on the current user in OursPrivacy People, but doesn't overwrite if
    ...
    /**
     Delete current user's record from OursPrivacy People.
     */
