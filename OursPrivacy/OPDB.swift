...
class OPDB {
    ...
    private let DB_FILE_NAME: String = "OPDB.sqlite"
    ...
    private func tableNameFor(_ persistenceType: PersistenceType) -> String {
        return "ours_\(apiToken)_\(persistenceType)"
    }
    
    private func reconnect() {
        OursPrivacyLogger.warn(message: "No database connection found. Calling OPDB.open()")
        open()
    }
    
    func open() {
        if apiToken.isEmpty {
            OursPrivacyLogger.error(message: "Project token must not be empty. Database cannot be opened.")
            return
        }
        if let dbPath = pathToDb() {
            if sqlite3_open_v2(dbPath, &connection, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nil) != SQLITE_OK {
                ...
                OursPrivacyLogger.info(message: "Successfully opened connection to database at path: \(dbPath)")
                if let db = connection {
                    ...
                    if sqlite3_prepare_v2(db, pragmaString, -1, &pragmaStatement, nil) == SQLITE_OK {
                        if sqlite3_step(pragmaStatement) == SQLITE_ROW {
                            ...
                            OursPrivacyLogger.info(message: "SQLite journal mode set to \(res)")
                        } else {
    ...
    func close() {
        ...
        connection = nil
        OursPrivacyLogger.info(message: "Connection to database closed.")
    }
        ...
        if let dbPath = pathToDb() {
            do {
                ...
                if manager.fileExists(atPath: dbPath) {
                    ...
                    OursPrivacyLogger.info(message: "Deleted database file at path: \(dbPath)")
                }
            } catch let error {
                OursPrivacyLogger.error(message: "Unable to remove database file at path: \(dbPath), error: \(error)")
            }
        }
        reconnect()
    }
        ...
        if let db = connection {
            ...
            if sqlite3_prepare_v2(db, createTableString, -1, &createTableStatement, nil) == SQLITE_OK {
                if sqlite3_step(createTableStatement) == SQLITE_DONE {
                    OursPrivacyLogger.info(message: "\(tableName) table created")
                } else {
        ...
        if let db = connection {
            ...
            if sqlite3_prepare_v2(db, createIndexString, -1, &createIndexStatement, nil) == SQLITE_OK {
                if sqlite3_step(createIndexStatement) == SQLITE_DONE {
                    OursPrivacyLogger.info(message: "\(indexName) index created")
                } else {
...
//        createIndexFor(PersistenceType.groups)
                ...
                if let pointer = rawBuffer.baseAddress {
                    if sqlite3_prepare_v2(db, insertString, -1, &insertStatement, nil) == SQLITE_OK {
                        ...
                        if sqlite3_step(insertStatement) == SQLITE_DONE {
                            OursPrivacyLogger.info(message: "Successfully inserted row into table \(tableName)")
                        } else {
        ...
        if let db = connection {
            ...
            if sqlite3_prepare_v2(db, deleteString, -1, &deleteStatement, nil) == SQLITE_OK {
                if sqlite3_step(deleteStatement) == SQLITE_DONE {
                    OursPrivacyLogger.info(message: "Successfully deleted rows from table \(tableName)")
                } else {
        ...
        if let db = connection {
            ...
            if sqlite3_prepare_v2(db, updateString, -1, &updateStatement, nil) == SQLITE_OK {
                if sqlite3_step(updateStatement) == SQLITE_DONE {
                    OursPrivacyLogger.info(message: "Successfully updated rows from table \(tableName)")
                } else {
    ...
    func readRows(_ persistenceType: PersistenceType, numRows: Int, flag: Bool = false) -> [InternalProperties] {
        ...
        if let db = connection {
            ...
            if sqlite3_prepare_v2(db, selectString, -1, &selectStatement, nil) == SQLITE_OK {
                ...
                if rowsRead > 0 {
                    OursPrivacyLogger.info(message: "Successfully read \(rowsRead) from table \(tableName)")
                }
            } else {
    ...
    private func logSqlError(message: String? = nil) {
        if let db = connection {
            if let msg = message {
                OursPrivacyLogger.error(message: msg)
            }
            let sqlError = String(cString: sqlite3_errmsg(db)!)
            OursPrivacyLogger.error(message: sqlError)
        } else {
            reconnect()
        }
    }
