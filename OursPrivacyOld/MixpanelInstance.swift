...
public protocol OursPrivacyDelegate: AnyObject {
     ...
     */
    func oursprivacyWillFlush(_ mixpanel: OursPrivacyInstance) -> Bool
}
...
open class OursPrivacyInstance: CustomDebugStringConvertible, FlushDelegate, AEDelegate {
    ...
    let mixpanelPersistence: OursPrivacyPersistence
    ...
    open var loggingEnabled: Bool = false {
        didSet {
            if loggingEnabled {
                ...
                OursPrivacyLogger.info(message: "MixpanelLogging Enabled")
            } else {
                OursPrivacyLogger.info(message: "MixpanelLogging Disabled")
    ...
    private init(
        ...
        mixpanelPersistence = OursPrivacyPersistence.init(instanceName: name)
...
extension OursPrivacyInstance {
    ...
    func unarchive() {
        let didCreateIdentity = self.readWriteLock.write {
            ...
            let mixpanelIdentity = OursPrivacyPersistence.loadIdentity(instanceName: self.name)
...
extension OursPrivacyInstance {
    ...
    public func track(event: String?, properties: Properties? = nil) {
        ...
        trackingQueue.async { [weak self, event, properties, epochInterval] in
            ...
            let mixpanelIdentity = OursPrivacyIdentity.init(distinctID: self.distinctId,
